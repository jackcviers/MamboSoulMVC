...
init=TRUE;imagesInSlidesLoaded=[];runAfterAnimationCallbacks=[];awaitingAjaxCallbacks=[];startedAjaxLoads=[];finishedAjaxLoads=[];slidesContainer=childrenNotAnimationClones(obj);var ulLength=slidesContainer.length;var newUl=$(DIV_TAG);if(!ulLength){obj.append(slidesContainer=newUl);isSlideContainerUl=FALSE;}else if(!(isSlideContainerUl=slidesContainer.is("ul"))&&!slideContainerCreated){newUl.append(slidesContainer);obj.append(slidesContainer=newUl);}
slideContainerCreated=TRUE;var slidesJquery=childrenNotAnimationClones(slidesContainer);slides=[];totalSlides=slidesJquery.length;slidesJquery.each(function(index,elem){var that=$(elem);slides[index]=that;that.css({position:RELATIVE_STRING});if(that.css("display")=="none"){that.css("display","inline");}});slidesContainer.addClass("slidesContainer");slidesJquery.addClass("slide");slidesJquery.each(function(index,elem){$(elem).attr("data-slide",index+1);});if(option[31]){var numerOfAjaxUrls=option[31].length;if(numerOfAjaxUrls>totalSlides){for(var a=1;a<=numerOfAjaxUrls-totalSlides;a++){var tag;if(isSlideContainerUl){tag="li";}else{tag="div";}
...
if(option[2]){bindAndRegisterOff(doc,"click",customLinkClickHandler,option[2]);}
...
var newWidth=getResponsiveWidth();if(previousAdjustedResponsiveWidth!=newWidth||(forced===TRUE)){previousAdjustedResponsiveWidth=newWidth;for(var i=0;i<totalSlides;i++){slides[i].width(newWidth);}
...
function getResponsiveWidth(){var width=obj.width();if(option[7]){return width;}else{return width/numberOfVisibleSlides;}}
...
stopAuto();autoOn=TRUE;autoStartedWithPause=pause;autoTimeout=schedule(function(){if(autoOn){enqueueAnimation(NEXT_STRING,FALSE);autoStartedWithPause=FALSE;}},pause);}
...
if(option[39]){animate(fadeElement,adjustObject,fadetime,option[47],callback);}else{fadeElement.animate(adjustObject,{queue:FALSE,duration:fadetime,easing:option[12],callback:callback});}}
...
if(option[39]){animate(obj,adjustObject,speed,option[47])}else{if(speed==0){obj.stop().css(adjustObject);}else{obj.animate(adjustObject,{queue:FALSE,duration:speed,easing:option[12]});}}}
...
function getSlidePosition(slide,vertical){if(vertical==undefined){vertical=option[7];}
...
if(direction!=0){currentTouchSlide+=direction;if(option[7]){initialOffsetTop-=rightSlidePositionBefore-rightSlidePositionAfter;}else{initialOffsetLeft-=rightSlidePositionBefore-rightSlidePositionAfter;}
distanceToRightSlide=getSlidePosition(currentTouchSlide+1)-getSlidePosition(currentTouchSlide);}}
if(option[7]){x=0;}else{y=0;}
...
if(type==startEvent){startX=x;startY=y;touchStart(x-startX,y-startY)}else{touchMove(x-startX,y-startY);if(!allowScroll(isMouseEvent,x-startX,y-startY)){event.preventDefault();}}
prevX=x-startX;prevY=y-startY;}else{touchEnd(prevX,prevY);startedTouch=FALSE;}};bindMultiple(doc,dragFunction,[TOUCHSTART,TOUCHMOVE,TOUCHEND,TOUCHCANCEL]);if(option[45]){bindMultiple(doc,dragFunction,[MOUSEDOWN,MOUSEMOVE,MOUSEUP]);}}
function allowScroll(isMouseEvent,x,y){var isGeneralDirectionVertical=mathAbs(y)>mathAbs(x);if(isMouseEvent){return FALSE;}
if(!option[46]){return FALSE;}
if(option[7]){return!isGeneralDirectionVertical;}else{return isGeneralDirectionVertical;}}}
...
var diff=dir-currentSlide;var targetSlide;if(option[16]&&!prevNext){var diffAbs=mathAbs(diff);targetSlide=dir;var newDiff=-currentSlide+dir+totalSlides;if(mathAbs(newDiff)<diffAbs){targetSlide=dir+totalSlides;diff=newDiff;diffAbs=mathAbs(diff);}
...
baseSlider.init=publicInit;baseSlider.getOption=function(a){return options[a.toLowerCase()];};baseSlider.setOption=function(key,val){publicDestroy();if($.isPlainObject(key)){for(var a in key){options[a.toLowerCase()]=key[a];}}else{options[key.toLowerCase()]=val;}
...
elem.css(CSSObject);callAsync(function(){elem.css(properties);var startTime=getTimeInMillis();elem.on(events,function(event){if(elem.is(event.target)&&(getTimeInMillis()-startTime)-speed>-100){callbackFunction();}});schedule(callbackFunction,speed+100);});});return callbackFunction}
...
return dir;}
function createLazyBoxes(obj,numberOfCols,numberOfRows,useToSlides){var slider=obj.slider;var vertical=obj.options.vertical;var result=[];var width=vertical?obj.slider.width():0;var height=vertical?0:obj.slider.height();obj.toSlides.each(function(){var that=$(this);if(vertical){height+=that.height();}else{width+=that.width();}});var boxWidth=Math.ceil(width/numberOfCols);var boxHeight=Math.ceil(height/numberOfRows);for(var row=0;row<numberOfRows;row++){for(var col=0;col<numberOfCols;col++){doStuff(row,col);}}
...
function getEffectMethod(inputEffect){if(isArray(inputEffect)){return makeRandomEffect(inputEffect);}else if(isFunction(inputEffect)){return inputEffect}else{inputEffect=stringTrim(inputEffect);if(inputEffect.indexOf(",")!=-1){var array=inputEffect.split(",");return makeRandomEffect(array);}else{var effects=objectToLowercase($.fn.sudoSlider.effects);var effectName=stringTrim(inputEffect.toLowerCase());var result=effects[effectName];if(result){return result;}else{var array=[];for(var name in effects){if(matches(name,effectName)){array.push(effects[name]);}}
